...
#define maybe_mutex_lock_scoped(ptr) mutex_lock_scoped(ptr)
...
static dirent_t *smb_readdir(void *hnd)
{
	struct smb_fd *fd = hnd;
	struct smb2dirent *dir;

	maybe_mutex_lock_scoped(&lock);

	dir = smb2_readdir(cxt, fd->hdl);
	if (!dir) {
		printf("Unable to readdir: %s\n", smb2_get_error(cxt));
		return NULL;
	}
...
int smb_init(const char *url)
{
	...
	if (!cxt) {
		printf("Unable to init libsmb2\n");
		return -EIO;
	}

	smb_url = smb2_parse_url(cxt, url);
	if (!smb_url) {
		printf("Could not parse SMB URL: %s\n", smb2_get_error(cxt));
	...
	ret = smb2_connect_share(cxt, smb_url->server,
				 ...
				 smb_url->user);
	if (ret) {
		printf("Could not connect to SMB share: %d\n", ret);
