...
static inline bool clut_is_outdated(const struct texture_clut *clut, bool bpp4)
{
	...
	unsigned int start, end, i, nb_colors = bpp4 ? 16 : 256;

	start = clut_get_texture_page(clut->clut);
	end = clut_get_texture_page(clut->clut + nb_colors / 16 - 1);

	for (i = start; i <= end; i++) {
		page = &pvr.textures4[i].base;
